function x = feasible(x,data)
    ...
    ir = cell(1,data.l);
    id = cell(1,data.l);
    newx = cell(1,data.l);A = 0; B = 0;C = 0;
    for j = 1 : data.l
        ...
        for i = 1: len
            ...
            if i < len
                for l  = 1: numel(a(i)+1:a(i+1)-1)
                    if time_c(data.tw{j}(x.route{j}(a(i)+l)-data.M,:),x.arrtime(x.route{j}(a(i)+l))-data.M) < data.alpha
                        ir{j} = [ir{j}, x.route{j}(a(i))];
                        C = 1;
                        break;
                    end
                end      
        ...
        if ~isempty(ir{j}) && isempty(id{j}) %车辆超过容量，仓库未超过容量，重新安排该仓库的车辆路径
            for k = 1 : numel(unique(ir{j})) 
                customer = data.customer(newx{j}{ir{j}(k)}-data.M,:);
                demand = data.demand{j}(newx{j}{ir{j}(k)}-data.M,:);
                Q = data.Q(j);
                tw = data.tw{j}(newx{j}{ir{j}(k)}-data.M,:);
                time = data.time{j}([ir{j}(k),newx{j}{ir{j}(k)}],[ir{j}(k),newx{j}{ir{j}(k)}]);
                D = data.D{j}([ir{j}(k),newx{j}{ir{j}(k)}],[ir{j}(k),newx{j}{ir{j}(k)}]);
                [r,t] = vrptw(customer,demand,Q,tw,time,data.DPI,data.alpha,D);
                rou = [ir{j}(k),newx{j}{ir{j}(k)}];
                route{j}{ir{j}(k)} = rou(r);
                x.arrtime(j,newx{j}{ir{j}(k)}-data.M) = t;
            end
            ...
            for l = 1:numel(dep)
                ...
                time = data.time{j}([dep(l),newx{j}{dep(l)}],[dep(l),newx{j}{dep(l)}]);
                D = data.D{j}([dep(l),newx{j}{dep(l)}],[dep(l),newx{j}{dep(l)}]);
                [r,t] = vrptw(customer,demand,Q,tw,time,data.DPI,data.alpha,D);
        ...
        elseif ~isempty(id{j}) %仓库超过容量，剔除超过的一部分客户，再重新安排路径
            for k = 1 : numel(unique(id{j}))
                ...
                while Cr(p{j}{id{j}(k)} ,p1(id{j}(k),:)) < data.API
                    ...
                    if dema(1,3) + d(3) >  max(p2(:,3))
                        dema(2,:) = dema(2,:) + d;
                        cust{2} = [cust{2},newx{j}{id{j}(k)}(end)];
                    else
                        cust{1} = [cust{1},newx{j}{id{j}(k)}(end)];
                        dema(1,:) = dema(1,:) + d;
                    end
                    newx{j}{id{j}(k)}(end) = [];
                end
                for u = 1: numel(cust)
                    if ~isempty(cust{u})
                        dd = dema(u,:);
                        depot1 = find(sum((p2 - dd(end:-1:1) >=0)') == 3);
                        index = depot1(randi(numel(depot1)));
                        newx{j}{index} = [newx{j}{index},cust{u}];
                        customer = data.customer(newx{j}{index}-data.M,:);
                        demand = data.demand{j}(newx{j}{index}-data.M,:);
                        Q = data.Q(j);
                        tw = data.tw{j}(newx{j}{index}-data.M,:);
                        time = data.time{j}([index,newx{j}{index}],[index,newx{j}{index}]);
                        D = data.D{j}([index,newx{j}{index}],[index,newx{j}{index}]);
                        [r,t] = vrptw(customer,demand,Q,tw,time,data.DPI,data.alpha,D);
