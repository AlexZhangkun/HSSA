...
for i = 1: numel(F1) 
    ...
    for k = 1 : numel(point)
        ...
        if length(x)>1
            q = data.Q; p = data.P; 
            t = F1(i).time(x(2)-data.M);%初始时间点 
            for l = 2 : numel(x)
                if q >= data.actualDemand(x(l)-data.M) && p(x(1)) >= data.actualDemand(x(l)-data.M) %不考虑仓库容量不足的情况，不符合实际
                    ...
                    satisfactory(x(l)-data.M) = satisfactory(x(l)-data.M) + time_c(data.tw(x(l)-data.M,:),time(x(l)-data.M));%计算服务满意度
                elseif q < data.actualDemand(x(l)-data.M) && p(x(1)) >= data.actualDemand(x(l)-data.M) %车辆剩余容量不足以服务客户但是仓库足够
                    ad_distance = ad_distance + 2 * data.D(x(l),x(1)); %额外行驶距离
                    t = t + 2 * data.time(x(l),x(1));
                    time(x(l)-data.M) = t; %时间窗变动 
                    t = t + data.tw(x(l)-data.M,5);
                    satisfactory(x(l)-data.M) = satisfactory(x(l)-data.M) + time_c(data.tw(x(l)-data.M,:),time(x(l)-data.M));%服务满意度变动
                    p(x(1)) = p(x(1)) - data.actualDemand(x(l)-data.M);
