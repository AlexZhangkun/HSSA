...
for i = 1 : data.l
    ...
    data.p2c(i,:) = dis1(data.park,[data.depot{i};data.customer]);
...
% [x,x_child, F, data] = NS([x;x_child], data, Num+Num*c_num,c_num); %非支配解排序nsga3
                ...
                for j = 1 : c_num
                    if dominate(x(i),x(k)) ~= 0
                        ...
                        if rand <=0.5           %x(i),x((i-1)*c_cum + j)
                            ...
                            no = F{1}(randperm(length(F{1}),1));
                            if no > numel(x)
                                E = x_child(no-numel(x));
                            else
                                E = x(numel(x));
                            end
